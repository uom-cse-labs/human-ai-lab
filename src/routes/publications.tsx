...
function PublicationsPage() {
  ...
  const [activeReaderPaper, setActiveReaderPaper] =
    useState<Publication | null>(PUBLICATIONS[0] || null);
  ...
  useEffect(() => {
    if (openSearch) {
      setShowSearchBox(true);
      navigate({
        search: (prev) => ({ ...prev, openSearch: false }),
        replace: true,
      });
      const t = setTimeout(() => {
        searchInputRef.current?.focus();
  ...
  useEffect(() => {
    if (
      activeReaderPaper &&
      !filteredPapers.find((p) => p.id === activeReaderPaper.id)
    ) {
      ...
      <section className="w-full relative bg-surface border-b border-outline overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 pt-32 pb-24 relative z-10 flex flex-col gap-6">
          ...
          <p className="font-sans text-lg md:text-xl text-on-background/60 leading-[1.6] max-w-3xl mt-4">
            Explore our complete archive of peer-reviewed research presented at
            leading global conferences including NeurIPS, CHI, and AAAI. Our
            papers reflect our commitment to open, transparent, and
            ethically-grounded AI research.
          </p>
        </div>
      </section>
              ...
              <div id="search" className="flex items-center justify-between">
                ...
                {!showSearchBox ? (
                  ...
                  <div className="flex items-center gap-2 bg-surface border border-outline py-1.5 px-3 rounded-full animate-fade-in w-full max-w-[200px]">
                    ...
                    <button
                      onClick={clearSearch}
                      className="text-on-background/50 hover:text-on-background"
                    >
                ...
                filteredPapers.map((paper) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3
                        className={`font-sans text-base font-black leading-[1.3] tracking-tight uppercase line-clamp-2 mb-2 transition-colors ${isActive ? "text-on-background" : "text-on-background group-hover:text-primary"}`}
                      >
                        {paper.title}
                      </h3>
            ...
            {activeReaderPaper ? (
              <div className="bg-surface border border-outline rounded-3xl p-8 md:p-12 animate-fade-in relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <Badge
                    variant="outline"
                    className="font-mono text-[10px] tracking-widest uppercase border-primary/30 text-primary mb-6"
                  >
              ...
              <div className="h-full min-h-[60vh] bg-surface/30 border border-dashed border-outline rounded-3xl flex flex-col items-center justify-center p-12 text-center">
                ...
                <h3 className="font-sans text-xl font-black text-on-background/40 uppercase">
                  No Paper Selected
                </h3>
                <p className="font-sans text-sm text-on-background/30 mt-2">
                  Select a publication from the index to view details.
                </p>
              </div>
            )}
          </div>
