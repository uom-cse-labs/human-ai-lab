    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <section className="w-full relative bg-surface border-b border-outline overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-1/2 h-full bg-[#F27D26] blur-[150px] rounded-full transform translate-x-1/2 -translate-y-1/2" />
        </div>
        <div className="max-w-7xl mx-auto px-6 pt-32 pb-24 relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-12">
          <div className="max-w-4xl">
            <span className="font-mono text-[10px] font-black uppercase tracking-[0.25em] text-[#F27D26] mb-6 block border border-[#F27D26]/30 bg-[#F27D26]/10 w-fit px-3 py-1 rounded-full">
              ABOUT THE AIM LAB
            </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          <div className="lg:col-span-5">
            ...
            <div className="w-full aspect-[4/3] bg-surface-dim border border-outline rounded-2xl overflow-hidden relative">
              ...
              />
              <div className="absolute inset-0 bg-[#F27D26]/10" />
            </div>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col justify-center">
            ...
            <div className="bg-surface p-10 rounded-2xl border-l-4 border-[#F27D26]">
              <span className="font-mono text-[10px] font-black tracking-widest text-[#F27D26] uppercase mb-4 block">
                THE LAB DIRECTIVE
              </span>
      ...
      <section className="max-w-7xl mx-auto px-6 py-28 border-b border-outline">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {methodology.map((step, idx) => (
            <div key={idx} className="flex gap-6 p-8 bg-surface border border-outline rounded-2xl group hover:border-[#F27D26]/50 transition-colors">
              <div className="flex-shrink-0 w-16 h-16 bg-background border border-outline rounded-xl flex items-center justify-center text-[#F27D26] group-hover:scale-110 transition-transform">
      ...
      <section className="max-w-7xl mx-auto px-6 py-28 border-b border-outline">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {pillars.map((pillar, idx) => (
            <div key={idx} className="group flex flex-col p-8 bg-background border border-outline hover:border-[#F27D26]/50 transition-colors rounded-2xl">
              <div className="w-14 h-14 flex items-center justify-center bg-surface text-[#F27D26] border border-outline rounded-2xl mb-8 group-hover:bg-[#F27D26] group-hover:text-black transition-colors">
      ...
      <section className="max-w-7xl mx-auto px-6 mt-28">
        <div className="bg-[#F27D26] p-12 md:p-20 rounded-3xl text-center relative overflow-hidden">
          <div className="absolute inset-0 bg-black/10 mix-blend-overlay" />
          <div className="relative z-10 max-w-3xl mx-auto">
            <h2 className="font-sans text-4xl md:text-6xl font-black tracking-tighter text-black uppercase leading-[0.95] mb-8">
              Let's build a predictable future together.
            </h2>
            <p className="font-sans text-lg md:text-xl text-black/80 font-medium mb-12">
              Whether you're a prospective PhD student, an industry partner, or a fellow researcher, we're always open to collaboration.
            </p>
            <button className="bg-black text-[#F27D26] font-sans font-black uppercase tracking-wider px-8 py-4 rounded-full hover:bg-white hover:text-black transition-colors">
              Contact the Lab
            </button>
          </div>
        </div>
