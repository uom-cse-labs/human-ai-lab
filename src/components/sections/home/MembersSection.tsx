import { TEAM_MEMBERS, FOUNDING_RAS, STUDENT_RESEARCHERS, ALUMNI } from '@/data';
...
function SocialLinks({ socials, size = 'md' }: SocialLinksProps) {
  if (!socials || (!socials.email && !socials.linkedin && !socials.scholar)) return null;
  const isSm = size === 'sm';
  const btnClass = isSm
  ...
  return (
    <div className={`flex items-center gap-1.5 ${isSm ? 'mt-2' : 'mt-3'}`} onClick={(e) => e.stopPropagation()}>
      {socials.email && (
      ...
      {socials.linkedin && (
        <a href={socials.linkedin} target="_blank" rel="noopener noreferrer" className={btnClass} title="LinkedIn">
          <Linkedin className={iconSize} />
        </a>
      )}
      {socials.scholar && (
        <a href={socials.scholar} target="_blank" rel="noopener noreferrer" className={btnClass} title="Google Scholar">
            ...
            >
              ...
              <div className="lg:col-span-9 flex flex-col justify-between py-2">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mt-4">
                  ...
                  <div className="flex items-center gap-3 self-end md:self-auto shrink-0" onClick={(e) => e.stopPropagation()}>
